...
export interface ProgressiveFluxLoaderProps {
  ...
  showLabel?: boolean;
  /**
   * CSS background for the bar fill. Defaults to the signature flux gradient.
   * Recolor via `--flux-from` / `--flux-to` CSS variables on a parent element.
   */
  gradient?: string;
...
const FLUX_FROM = 'var(--flux-from, #e07d00)';
const FLUX_TO = 'var(--flux-to, #fa9f1a)';
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    'absolute inset-0 flex items-center justify-center text-center text-lg font-semibold tracking-tight text-muted-foreground sm:text-xl',
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'relative h-2.5 w-full overflow-hidden rounded-full bg-muted shadow-[inset_0_1px_2px_rgba(0,0,0,0.09),inset_0_-1px_1px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.45),inset_0_-1px_1px_rgba(255,255,255,0.05)]',
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
